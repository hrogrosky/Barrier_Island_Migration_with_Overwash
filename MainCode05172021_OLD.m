...
if restartrun==0
    ...
    %%% WHICH ISLAND %%%
    islandstring='S';     % Use 'P' for Parramore; 'S' for Smith
    
    %%% WHICH VIMS SLR ACCELERATION CURVE %%%
    SLRrun=1;  %1 - historic; 2 - low; 3 - high; 4 - highest.  Based on SLR graph from VIMS
    
    %%% BRUUN RULE COEFFICIENT
    BruunParam=4000;  % Suggested values:  500, 1000, 1500, 2000, 2500
    
    %%% DIRECTORY WHERE MAINCODE FILE IS KEPT
    cd '/Users/beththomas/Documents/MATLAB/Barrier Island/';
    
    %%% DETERMINE HOW OFTEN TO PLOT AND/OR SAVE DATA %%%
    DurImFreq=26;       %how often to output "during" elevation images (26~yearly, 130~every 5 years)
        MElevCont=0;
        SElevCont=0;
    PlantDurImFreq=26; %same, just for plant images
    DurCUImFreq=26;
    DursavedataFreq=26; %how often to save data (in 2-week intervals; 26~yearly, 78~every 3 years, 130~every 5 years)
    ...
    if islandstring=='P'
        ...
        filename='ParramoreElevMap05252021.mat'
    ...
    fprintf('Initializing...')
               %cheat sheet:
    time=26*5; %8*26+702;  %2600~100yrs;%1300~50yrs;%780~30yrs;%520~20yrs;260~10yrs;
    delta=0.1;
    % L=1;%<----------ScaleFactor
    L=ScaleFactor;
    ...
    MasterMax=1.0;% The most any cell can permit - 80% plant coverage
...
%     gdrange2=[-.02:.01:.08];%[-.04:.01:.04]; %range of percent values for growth/death for plant pops greater than 50% cover
    ...
    DurImFreq=130;      %how often to output "during" elevation images (26~yearly, 130~every 5 years)
        MElevCont=0;
        SElevCont=0;
...
if ElevIm_before==1
    ...
    hold on
    if MElevCont==1
    contour(H(:,:,1),[-5 -5],'color','m','linewidth',1.1)
    end
    if SElevCont==1
...
%                 end
 ...
 %this code is set up to move 2 cells at time

if (0==mod(t,26)) %runs once a year
 [Hstar]=TipsCode10052022(H); 
end
H = Hstar; %new matrix Hstar stores this movement
