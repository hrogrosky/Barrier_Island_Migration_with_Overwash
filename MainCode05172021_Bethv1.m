...
if restartrun==0
    ...
    BruunParam=0;  % Suggested values:  500, 1000, 1500, 2000, 2500 %was 4000 on 10/3/23

    %%% DUNE CREST WINDOW IN OVERWASH
    window = 400;
    window_top = 400;
    ...
    DurImFreq=26*30;       %how often to output "during" elevation images (26~yearly, 130~every 5 years)
        MElevCont=1; %make 1 if you want contour
        SElevCont=0;
    PlantDurImFreq=0; %same, just for plant images %WAS 130
    DurCUImFreq=130;
    DursavedataFreq=130; %how often to save data (in 2-week intervals; 26~yearly, 78~every 3 years, 130~every 5 years)
    Dursavedatavec = [0, 26*5, 26*10, 26*14, 26*15, 26*20, 26*25, 26*27, 26*30, 26*35, 26*40, 26*45, 26*50];
...
%         filename='ParramoreElevMap07082024.mat'; %including all tiny islands
        filename = 'Parramore2021ElevMap.mat';
        IslandArea=19070000; %for parramore
        data=importdata(filename);
        H=data;                         %H will be the main elevation matrix
        TranChk=1;
        Plx_scale = 15*10^3; %in marine processes
        top_transect1 = 500; %transects stored in H_transects
        top_transect2 = 750;
        mid_transect1 = 1250;
        mid_transect2 = 1750;
        bot_transect1 = 2250;
        bot_transect2 = 2600;
        k1 = 0.5; %cells removed in overwash
        k2 = 0.06; %cells removed in erosion
        SS_scale = 1;
...
%         filename='SmithNew06132021.mat';
        filename='Smith2021ElevMap.mat';
%         IslandArea=9065000; %for Smith
        IslandArea=269949; %for Smith
        data=importdata(filename);
        H=data;                         %H will be the main elevation matrix
        TranChk=2;
        Plx_scale = 2.42*10^5;
        ...
        bot_transect2 = 2250;
        k1 = 0.5; %cells removed in overwash
        k2 = 0.006; %cells removed in erosion
        SS_scale = 0.6;
    end
...
%%% ALSO REDUCES SIZE OF SMITH 2021 INITIAL CONDITIONS %%%%
...
H = A2;
    ...
    fprintf('Initializing...')
               %cheat sheet:
    time= 26*30; %26*10; %8*26+702;  %2600~100yrs;%1300~50yrs;%780~30yrs;%520~20yrs;260~10yrs;
    delta=0.1;
    % L=1;%<----------ScaleFactor
    L=ScaleFactor;
    ...
    DC_top = zeros(time+1,3); %initiates matrix to store DC location at top transect at each time step
    DC_bottom = zeros(time+1,3); %initiates matrix to store DC location at bot transect at each time step
    count_hist = zeros(time+1,3); %initiates matrix to store counts of sand removed at top/bot transect
    %OW_hist = zeros(n1,1); %initiates vector tha counts how many times overwash ran in each row
    ...
    % edited on 9/11/24 to save P or S, time end, date, and time code ran
    filenamestring=char(strcat([islandstring,'te',num2str(time),'_'])+string(datetime('now')));
...
%     PCmp=(P1ErosionCoefficient.*Pt1)+(P2ErosionCoefficient.*Pt2)+(P3ErosionCoefficient.*Pt3)+(P4ErosionCoefficient.*Pt4);
    [Hstar,P3,MeanBeachWidth,ESL,WSL,MESL,MWSL,OppositeLocation,SLRyrs,MigCnt,MigAccel]=MarineProcesses05282021(Hstar,delta,L,P1,P2,P3,P4,BchMax,OW,t,SLRyrs,PCmp,IslandArea,MigCnt,ScaleFactor,MigYr,Ma,MigAccel,MasterMax,SLRvec,BruunParam,Plx_scale);
...
fprintf('Initialization complete')
    ...
    top_hist1 = zeros(time+1,n2);
    mid_hist1 = zeros(time+1,n2);
    bot_hist1 = zeros(time+1,n2);
    top_hist2 = zeros(time+1,n2);
    mid_hist2 = zeros(time+1,n2);
    bot_hist2 = zeros(time+1,n2);
...
[H_overwash, PC1_overwash, PC2_overwash, PC3_overwash, PC4_overwash, DC, n1, surge_hist, EastSL, DC_top, DC_bottom, count_hist,OW_hist]=Overwash072324_v2(Hstar, PC, PC1, PC2, PC3, PC4, time, t, surge_hist, DC_top, DC_bottom, count_hist,window,window_top,OW_hist,k1, k2, SS_scale); 
...
% ESL_hist = ESL_hist;
% 
[top_hist1, mid_hist1, bot_hist1, top_hist2, mid_hist2, bot_hist2, H_cell_count] = storing_H_transects(Hstar,t,top_hist1, mid_hist1, bot_hist1, top_hist2, mid_hist2, bot_hist2, H_cell_count,top_transect1, mid_transect1, bot_transect1, top_transect2, mid_transect2, bot_transect2);

%% Smoothing the shorelines every 5 years
if (0==mod(t,13)) && t~=0
    [Hstar,EastSL_change] = Shoreline_Smooth(Hstar);
end
...
%     [Hstar,P1,P2,P3,P4]=LandCategorizationUNUSED03312021(Hstar,P1,P2,P3,P4,PlantColumnArray,PlantColumnArray2,ColumnArraySwamp1,ColumnArraySwamp2,INum);
    ...
    if (0==mod(t,26))
        if MPswitch==1
            ...
            [Hstar,P3,MeanBeachWidth,ESL,WSL,MESL,MWSL,OL,SLRyrs,MigCnt,MigAccel]=MarineProcesses05282021(Hstar,delta,L,P1,P2,P3,P4,BchMax,OW,t,SLRyrs,PCmp,IslandArea,MigCnt,ScaleFactor,MigYr,Ma,MigAccel,MasterMax,SLRvec,BruunParam,Plx_scale);
        elseif MPswitch==2
            [Hstar,PlantColumnArray,PlantColumnArray2,P3,flag,MeanBeachWidth]=OLDMarineProcesses03312021(Hstar,delta,L,flag,PC,P3,BchMax,BchW);
        end
        if MPswitch~=0
...
%             scatter(DC, 1:n1,sz,'w', 'filled');
            ...
            if MElevCont==1 && SElevCont==1
                ...
                set(lgnd,'color','none','location','southeast');
            elseif MElevCont==1 && SElevCont==0
                lgnd=legend('\color{black} current island contour (0m)','\color{black} original island contour (0m)');
                set(lgnd,'color','none','location','southeast');
            elseif SElevCont==1 && MElevCont==0
                lgnd=legend('\color{white} sea level contour (0m)');
                set(lgnd,'color','none','location','southeast');
            ...
            cd(filenamestring);
            if saveasdotfig==1 saveas(gcf,figfilenamestring); end % save as .fig
            saveas(gcf,strcat([figfilenamestring,'.fig'])); % save as .jpg
            cd ..
            %%%%
...
save(strcat([filenamestring,'t',int2str(t/26),'.mat']),'H','Hstar','P1','P2','P3','P4','PC','top_hist1', 'top_hist2','bot_hist1', 'bot_hist2','mid_hist1', 'mid_hist2','surge_hist','count_hist','t','OW_hist')
        cd ..
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
