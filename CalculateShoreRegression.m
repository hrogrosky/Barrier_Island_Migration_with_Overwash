...
figure;
contourf(flipud(ParCrop(:,:,1)),'Linestyle','none');
...
for ii=1:m1
    for jj=m2:-1:2
        if Sv3fplot(ii,jj)<0 && Sv3fplot(ii,jj-1)>=0 && mim3(ii)==0
            mim3(ii)=jj-1;
        end
    end
end
for ii=1:m1
    for jj=m2:-1:2
        if Sv2fplot(ii,jj)<0 && Sv2fplot(ii,jj-1)>=0 && mim2(ii)==0
            mim2(ii)=jj-1;
        end
    end
end
for ii=1:m1
    for jj=m2:-1:2
        if Sv1fplot(ii,jj)<0 && Sv1fplot(ii,jj-1)>=0 && mim1(ii)==0
            mim1(ii)=jj-1;
        end
    end
end
...
disp(strcat(['Parr Mod, 1984-2011 Shoreline Reg., Lower ',num2str(mean(3.08*(mim1(2013:3167)-mim3(2013:3167)))/27),' m/yr']));
disp(strcat(['Parr Mod, 1984-1998 Shoreline Reg., Lower ',num2str(mean(3.08*(mim1(2013:3167)-mim2(2013:3167)))/14),' m/yr']));
disp(strcat(['Parr Mod, 1998-2011 Shoreline Reg., Lower ',num2str(mean(3.08*(mim2(2013:3167)-mim3(2013:3167)))/13),' m/yr']));
...
disp(strcat(['Smith Mod, 20 to 30 Shoreline Reg., Upper ',num2str(mean(4.04*(mim1(423:1388)-mim2(423:1388)))/10),' m/yr']));
disp(strcat(['Smith Mod, 30 to 40 Shoreline Reg., Upper ',num2str(mean(4.04*(mim2(423:1388)-mim3(423:1388)))/10),' m/yr']));

disp(strcat(['Smith Mod, 20 to 30 Shoreline Reg., Lower ',num2str(mean(4.04*(mim1(1388:2354)-mim2(1388:2354)))/10),' m/yr']));
disp(strcat(['Smith Mod, 30 to 40 Shoreline Reg., Lower ',num2str(mean(4.04*(mim2(1388:2354)-mim3(1388:2354)))/10),' m/yr']));

disp(strcat(['Smith Mod, 20 to 30 Shoreline Reg., All ',num2str(mean(4.04*(mim1(423:2354)-mim2(423:2354)))/10),' m/yr']));
disp(strcat(['Smith Mod, 30 to 40 Shoreline Reg., All ',num2str(mean(4.04*(mim2(423:2354)-mim3(423:2354)))/10),' m/yr']));
